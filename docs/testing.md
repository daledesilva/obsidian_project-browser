...
- **Helpers** — `tests/e2e/helpers/dismiss-popups.ts` (fallback for blocking notices), `tests/e2e/helpers/open-card-browser.ts` (`openCardBrowserFrom`, `openCardBrowserAndEnterFolder`)
...
| **new-tab-replacement.e2e.ts** | With `replaceNewTab: true`, creating a new leaf (e.g. `getLeaf(true)`) is replaced by the card browser view. |
...
All E2E specs use the same generated vault and call `dismissBlockingPopups()` in `before`. Flows that are hard to automate (e.g. ribbon icon if not exposed by the service) are left to manual QA and noted in the spec or docs.
